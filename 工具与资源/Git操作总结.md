...
```bash
git switch feature/select-card # 切换到feature/select-card分支
git diff master # 查看feature/select-card对比master分支做了哪些改动
```

能diff出来所有改动，包含已经commit、已经add的**和未add的**

⑥当改动很多时一行一行显示看着很累，一些更方便的查看diff的方式

```bash
git diff master --name-only	# 只看文件名，只列出改动了哪些文件，不显示具体内容
git diff master --stat # 显示每个文件改了多少行，末尾有总计
...
```bash
git branch --set-upstream-to=origin/feature/GGB feature/GGB
```
...
1. 分支命名要用`-`连接而不是`_`，例如`feature/equity-card-list`而不是`feature/equity_card_list`
